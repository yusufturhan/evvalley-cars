...
export default function UpdateVehiclePage() {
  ...
  useEffect(() => {
    console.log("UPDATE PAGE: Starting...");
    console.log("UPDATE PAGE: Params:", params);
    
    const id = params?.id;
    if (id) {
      console.log("UPDATE PAGE: Vehicle ID:", id);
      
      fetch(`/api/vehicles/${id}`)
        .then(res => {
          console.log("UPDATE PAGE: API Response Status:", res.status);
          return res.json();
        })
        .then(data => {
          console.log("UPDATE PAGE: API Data:", data);
          setData(data);
        })
        .catch(err => {
          console.log("UPDATE PAGE: API Error:", err);
        })
        .finally(() => {
          setLoading(false);
        });
    } else {
      console.log("UPDATE PAGE: No vehicle ID found");
      setLoading(false);
    }
  }, [params]);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-white rounded-lg shadow-lg p-8">
          <h1 className="text-3xl font-bold text-green-600 mb-6">✅ UPDATE VEHICLE PAGE</h1>
            ...
            {data && data.vehicle && (
              <div className="mt-6">
                <button 
                  ...
                  onClick={() => {
                    console.log("UPDATE PAGE: Edit button clicked");
